...
export function computeMetrics({ custo, preco, sellout, volume, icmsPercentual, icms_percentual }) {
  const icmsPct = icmsPercentual ?? icms_percentual ?? 0;
  const icmsValor = preco > 0 ? preco * (icmsPct / 100) : 0;
  const lucroUnitario = preco - custo - icmsValor;
  const margemAtual = preco > 0 ? (lucroUnitario / preco) * 100 : 0;
  const markup = custo > 0 ? (preco / custo - 1) * 100 : 0;
  const novoCusto = custo - sellout;
  const novoLucroUnitario = preco - novoCusto - icmsValor;
  const novaMargem = preco > 0 ? (novoLucroUnitario / preco) * 100 : 0;
